...
int open_epoch(unsigned int te)
{
	...
	head1.tag = uepoch ? TYPE_1_TAG_U : TYPE_1_TAG;
	head1.length = 0;
	head1.epoc = finalepoc;
	head1.bitsperentry = 49;
...
int close_epoch()
{
	...
	if (retval != sizeof(struct header_1))
		return 14; /* cannot write header */
	i = index1 * sizeof(unsigned int);
	retval = write(handle1, outbuf1, i);
	if (retval != i)
		return 15; /* cannot write content */
...
int main(int argc, char *argv[])
{
	...
	ibf2 = (char *)inbuffer;
	if (!inbuffer)
		return -emsg(6); /* cannot get inbuffer */
	...
	{ /* filling of input buffer */
		...
		if (debuglog)
			if (i1)
				fprintf(debuglog, "got leftover: i1= %d bytes /n", i1);
		...
		do
		{
			...
			if (t_new < t_old)
			{ /* do general comparison for neg differences */
			...
			{ /* epoch is changing */
				...
				if (((t_epoc - oldepoc) & 0x10000) && (oldepoc != 0))
				{
					...
					if (debuglog)
						fprintf(debuglog, "got neg tepoc: old: %08x new: %08x", oldepoc, t_epoc);
				...
				if ((t_epoc > oldepoc + 1) && (oldepoc != 0))
				{
					...
					if (debuglog)
						fprintf(debuglog, "got pos tepoc: old: %08x new: %08x", oldepoc, t_epoc);
