...
int smidx[7] = {15, 1, 2, 4,
                8,  3, 6};     /* output pattern - now six det capable */
      ...
      case 3:                        /* do complex log */
        if (fourdetectorlogoption) { /* old style logging */
          ...
          for (i = 0; i < 7; i++) {
            ...
            /* construct individual detector sums */
            for (j = 0; j < 16; j++)
              if ((i < 1) || (j == smidx[i])) sum[i] += detcnts[j];
          }
...
int main(int argc, char *argv[]) {
  ...
  while (1) { /* filling of input buffer */
    ...
    inbytesread =
        read(handlein, ibf2a, INBUFENTRIES * sizeof(struct rawevent) - i1);
    ...
    do {
      ...
      if (t_new < t_old) { /* do general comparison for neg differences */
        if ((t_new - t_old) & 0x1000000000000ll) { /* treat rollover */
          inpointer++;
          fprintf(stderr, "got negative difference: new: %0llx old: %0llx\n",
                  t_new, t_old);
          #ifdef DEBUG
          if (debuglog) {
            fprintf(debuglog,
        ...
        if (t_new > t_old + maxdiff) { /* most cases */
          if ((t_old - t_new + maxdiff) & 0x1000000000000ll) { /* rollover */
            if (t_old) { /* make sure to allow time diff at start */
              fprintf(stderr, "got pos difference: new: %016llx old: %016llx\n",
                      t_new, t_old);
              #ifdef DEBUG
              if (debuglog) {
                fprintf(debuglog,
      ...
      if (t_epoc - oldepoc) { /* epoch is changing */
        ...
        if (((t_epoc - oldepoc) & 0x10000) && (oldepoc != 0)) {
          ...
          inpointer++;
          fishyness++;
          fprintf(stderr, "got neg tepoc: old: %08x new: %08x", oldepoc,
                  t_epoc);
          #ifdef DEBUG
          if (debuglog)
        ...
        if ((t_epoc > oldepoc + 1) && (oldepoc != 0)) {
          ...
          inpointer++;
          fishyness++;
          fprintf(stderr, "got pos tepoc: old: %08x new: %08x", oldepoc,
                  t_epoc);
          #ifdef DEBUG
          if (debuglog)
