...
void findmax(int *buf1, int *buf2, double *maxval, double *sigma, double *mean,
             int *pos, int size, int ecnt1, int ecnt2) {
  int i;                 /* counter index */
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < epochnumber; i++) {
    ...
    epoch_offset =
        ((unsigned long long)((thisepoch + overlay_correction[overlay]) &
                              0xfffe0000))
        << 32;
  ...
  findmax(buf1_slow, buf2_slow, &maxval_s, &sigma_s, &mean_s, &pos_s, zhs,
          ecnt1, ecnt2);
  /* do job for fast array */
  findmax(buf1_fast, buf2_fast, &maxval_f, &sigma_f, &mean_f, &pos_f, zhs,
          ecnt1, ecnt2);
