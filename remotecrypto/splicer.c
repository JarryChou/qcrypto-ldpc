...
{ /* header for type-4 stream packet */
	int tag;
	unsigned int epoc;
	unsigned int length;
	int timeorder;
...
typedef struct protocol_details_C
{									   /* used in splicer program */
...
void FILL_TAB_3_4_HELP(unsigned int *t)
{ /* devindependent common part for 3/4.
					    argument
	...
	int i;
	for (i = 0; i < 128; i++)
		t[i] = 0x10; /* default pattern is ignore */
	/* all 16 bell test patterns */
	for (i = 0; i < 16; i++)
		t[((i & 3) << 4) + (1 << ((i >> 2) & 3))] = 0x20 + i;
	return;
}
void FILL_TABLE_PROTO3(unsigned int *t)
{						  /* version for local w 6 detectors */
...
void FILL_TABLE_PROTO4(unsigned int *t)
{ /* version for local w 4 detectors */
...
void FILL_TABLE_PROTO5(unsigned int *t)
{ /* BC version, copies everything */
	t[0] = 0;
	t[1] = 1;
	t[2] = 2;
	t[3] = 3;
...
int openmode[5] = {
	O_RDONLY, O_RDONLY,			  /* modes for input streams */
				 ...
				 struct header_3 *head)
{
	...
	if (!retval)
		return 20; /* nothing available */
	if (!(retval + 1))
		return 21; /* other error */
	if (retval < (int)sizeof(struct header_3))
		return 22;				   /* incomplete read */
	...
	if (bytenum * 4 != retval)
		return 24;
	/* protocol bit match? */
	if (h->bitsperentry != expected3bits)
		return 25;
	*head = h[0];
	return 0;
}
				 ...
				 struct header_4 *head, int *realsize)
{
	...
	if (!retval)
		return 26; /* nothing available */
	if (!(retval + 1))
		return 27; /* other error */
	if (retval < (int)sizeof(struct header_4))
		return 28;				   /* incomplete read */
	h = (struct header_4 *)buffer; /* at beginning of stream */
	/* consistency check on length and tag */
	if ((h->tag != TYPE_4_TAG) && (h->tag != TYPE_4_TAG_U))
		return 29;
	...
	if (h->length)
	{
		bitnum = (retval - sizeof(struct header_4)) * 8 / (h->length + 1) - h->basebits - h->timeorder;
		if ((bitnum < 0) | (bitnum > 32))
			return 30;
	}
	/* protocol bit match? */
	if (h->basebits != expected4bits)
		return 31;
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "V:i:d:I:D:o:f:e:q:p:kKl:E:L:m:b:B:")) != EOF)
	{
		...
		/* a funky way of parsing all file name options together.
		   ...
		   msb. */
		case 'f':
			i++; /* stream 3out, directory */
		case 'D':
			i++; /* stream 4in, directory */
		case 'd':
			i++; /* stream 3in, directory */
			i++;
		case 'o':
			i++; /* stream 3out, file */
		case 'I':
		...
		case 'i':		 /* stream 3in, file */
			...
			if (1 != sscanf(optarg, FNAMFORMAT, fname[j]))
				return -emsg(2 + j);
			fname[j][FNAMELENGTH - 1] = 0; /* security termination */
			if (typemode[j])
				return -emsg(5 + j); /* already defined mode */
			typemode[j] = (i & 4 ? 2 : 1);
			break;
		case 'B':
			i++;  /* stream 5 directory */
		case 'b': /* stream 5 outfile */
			if (1 != sscanf(optarg, FNAMFORMAT, fname[4]))
				return -emsg(43);
			fname[4][FNAMELENGTH - 1] = 0; /* security termination */
			if (typemode[4])
				return -emsg(44); /* already defined mode */
			typemode[4] = (i & 1 ? 2 : 1);
			break;
		...
		case 'E': /* command pipeline */
			if (1 != sscanf(optarg, FNAMFORMAT, fname[3]))
				return -emsg(38);
			cmdmode = 1;
			break;
		...
		case 'p': /* protocol index */
			if (1 != sscanf(optarg, "%i", &proto_index))
				return -emsg(10);
			if ((proto_index < 0) || (proto_index > PROTOCOL_MAXINDEX))
				return -emsg(11);
			break;
		case 'k':
		case 'K': /* kill mode stream 1 and 2 */
			killmode[opt == 'k' ? 0 : 1] = 1;
			break;
		case 'm':
			i++; /* generated type3 raw keys */
		case 'L':
			i++;  /* filename for consumed type4 packets */
	...
	{ /* open command pipeline */
		if (stat(fname[3], &cmdstat))
			return -emsg(40);
		if (!S_ISFIFO(cmdstat.st_mode))
			return -emsg(41);
		if (!(cmdhandle = fopen(fname[3], "r+")))
			return -emsg(39);
	}
	...
	if (!(buffer3i = (char *)malloc(RAW3i_SIZE)))
		return -emsg(13);
	if (!(buffer4i = (char *)malloc(RAW4i_SIZE)))
		return -emsg(14);
	if (!(buffer3o = (char *)malloc(RAW3o_SIZE)))
		return -emsg(15);
	if (!(buffer5o = (char *)malloc(RAW3o_SIZE)))
		return -emsg(45);
	...
	if (cmdmode)
	{ /* load initial epoch */
		if (1 != fscanf(cmdhandle, "%x", &current_ep))
			return -emsg(42);
	}
	else
	{
	...
	do
	{
		...
		/* prepare outstream-3 */
		head3o.tag = (head4i.tag == TYPE_4_TAG_U) ? TYPE_3_TAG_U : TYPE_3_TAG; /* same style */
		head3o.epoc = current_ep;
		head3o.length = 0;
		head3o.bitsperentry = type3odatawidth;

		/* prepare outstream-5 */
		head5o.tag = (head4i.tag == TYPE_4_TAG_U) ? TYPE_3_TAG_U : TYPE_3_TAG; /* same style */
		...
		ju = 0;
		readword4 = pointer4[ju++]; /* prepare read buffer */
		resbits4 = 32;
		type4bitwidth = head4i.timeorder;
		type4datawidth = head4i.basebits;
		...
		{ /* go through buffer */
			...
			k3i = tindex * type3ibits;
			n3i = k3i % 32; /* shift info sourceword */
			if ((u3i = k3i / 32) > stream3imaxindex)
				return -emsg(37);
			if (p3dec[n3i])
			{ /* contained in one word */
				pattern3i = (pointer3i[k3i / 32] >> p3sh0[n3i]) & patternmask3i;
			}
			else
			{
			...
			decpattern = lookup_table[pattern3i | (pattern4 << type3ibits)]; /* out pattern */
			if (!(decpattern & ignorepatternmask))
			{ /* don't ignore */
				if (decpattern & testbitmask)
				{ /* we have a stream-5 event */
		...
		if (resbits3o < 32)
			outbuf3o[index3o++] = sendword3; /* last word */

		/* eventually open output stream 3 */
		if (openstream(2, current_ep))
			return -emsg(19);
		/* write header 3 */
		...
		if (retval != i)
			return -emsg(33); /* write error buffer */
		...
		if (resbits5o < 32)
			outbuf5o[index5o++] = sendword5; /* last word */

		/* eventually open output stream 5 */
		if (openstream(4, current_ep))
			return -emsg(46);
		...
		if (typemode[4])
		{
			...
			if (retval != i)
				return -emsg(48); /* write error buffer */
		}
		/* eventually close stream 3 */
		if (typemode[4] == 2)
			close(handle[4]);
