...
void FILL_TAB_3_4_HELP(
    unsigned int *t) { /* devindependent common part for 3/4.
                                                                 argument
                                                        is remote response in
                          bits 5-7, and local detector pattern in bits 0-3. The
                          resulting word has the stream3/3 data in bits 0-3, for
                                                        bell data in the form
                          llrr to make the bell files on both sides identical.
                          bit 4 is the ignore indicator, and bit 5 the test
                          pattern indicator.
                                                      */
  int i;
...
char fname[5][FNAMELENGTH] = {"", "", "", "",
                              ""}; /* stream files / cmd pipeline */
...
int get_stream_3(void *buffer, int handle, int maxsize, struct header_3 *head) {
  int retval;
  int bytenum;
...
/* index file */
int get_stream_4(void *buffer, int handle, int maxsize, struct header_4 *head,
                 int *realsize) {
  int retval;
  int bitnum;
  struct header_4 *h;
...
int openstream(int i, unsigned int ep) {
  switch (typemode[i]) {
    case 2: /* file in directory */
      ...
      if (-1 == handle[i]) {
        fprintf(stderr, "handle %d named >>%s<< failed.", i, ffnam[i]);
        return 1;
      }
      break;
  }
...
int main(int argc, char *argv[]) {
  ...
  unsigned int readword4, emergency_break, pattern4, patternmask4,
      diff4_bitmask;
  int resbits4;
  ...
  while ((opt = getopt(argc, argv, "V:i:d:I:D:o:f:e:q:p:kKl:E:L:m:b:B:")) !=
         EOF) {
    ...
    switch (opt) {
      ...
      case 'l': /* read logfile name for consumed t3 intermed packets */
        if (sscanf(optarg, FNAMFORMAT, logfname[i]) != 1) return -emsg(12);
  ...
  /* fill outword table */
  lookup_table =
      (unsigned int *)malloc(sizeof(int) * proto_table[proto_index].decsize);
  ...
  do {
    ...
    /* load instream-4 */
    retval =
        get_stream_4(buffer4i, handle[1], RAW4i_SIZE, &head4i, &realsize4i);
    if (retval) return -emsg(retval);
    ...
    /* prepare outstream-3 */
    head3o.tag = (head4i.tag == TYPE_4_TAG_U) ? TYPE_3_TAG_U
                                              : TYPE_3_TAG; /* same style */
    head3o.epoc = current_ep;
    head3o.length = 0;
    head3o.bitsperentry = type3odatawidth;

    /* prepare outstream-5 */
    head5o.tag = (head4i.tag == TYPE_4_TAG_U) ? TYPE_3_TAG_U
                                              : TYPE_3_TAG; /* same style */
    ...
    do { /* go through buffer */
      ...
      decpattern =
          lookup_table[pattern3i | (pattern4 << type3ibits)]; /* out pattern */
  ...
  for (i = 0; i < 3; i++)
    if (logfname[i][0]) fclose(loghandle[i]);
  /* free buffers */
  free(buffer3i);
  free(buffer4i);
  free(buffer3o);
