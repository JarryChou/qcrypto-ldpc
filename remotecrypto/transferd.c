...
char fname[11][FNAMELENGTH] = {"", "", "", "", "", "", "", "", "", "", ""}; /* stream files */
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "d:c:t:D:l:s:km:M:p:e:E:b:")) != EOF)
	{
		...
		case 'd':
			...
			if (typemode[i])
				return -emsg(1 + i); /* already defined mode */
			typemode[i] = 1;
			break;
		case 'k': /* killmode */
			killmode = 1;
	...
	/* command pipe */
	if (access(fname[1], F_OK) == -1)
	{ /* fifo does not exist */
		if (mkfifo(fname[1], FIFOPERMISSIONS))
			return -emsg(17);
	}
	...
	if (typemode[6])
	{ /* message pipes exist */
		if (access(fname[6], F_OK) == -1)
		{ /* fifo does not exist */
			if (mkfifo(fname[6], FIFOPERMISSIONS))
				return -emsg(19);
		}
	...
	if (typemode[8])
	{ /* listen to it */
		if (access(fname[8], F_OK) == -1)
		{ /* fifo does not exist */
			if (mkfifo(fname[8], FIFOPERMISSIONS))
				return -emsg(11);
		}
	...
	if (typemode[9])
	{ /* open it */
		if (access(fname[9], F_OK) == -1)
		{ /* fifo does not exist */
			if (mkfifo(fname[9], FIFOPERMISSIONS))
				return -emsg(13);
		}
	...
	{
		switch (errno)
		{ /* gove perhaps some specific errors */
		default:
	...
	{				 /* while link should be maintained */
		...
		while (commskt)
		{ /* link is active,  wait on input sockets */
			FD_ZERO(&readqueue);
			FD_ZERO(&writequeue);
			fprintf(debuglog, "1\n");
			fflush(debuglog);
			...
			fflush(debuglog);
			if (!cmdmode)
				FD_SET(fileno(cmdhandle), &readqueue);
			fprintf(debuglog, "3\n");
			fflush(debuglog);
			if (sendbf)
				FD_SET(commskt, &writequeue); /* if we need to write */
			fprintf(debuglog, "4\n");
			fflush(debuglog);
			if (typemode[6] && !messagemode)
				FD_SET(msginhandle, &readqueue);
			fprintf(debuglog, "5\n");
			fflush(debuglog);
			...
			if (FD_ISSET(ercinhandle, &readqueue))
			{
				...
				case 1: /* finish reading header */
					...
					erci_idx += retval;
					if (erci_idx < sizeof(struct errc_header))
						break;
					/* got header, read data */
					if (ehead->length > LOC_BUFSIZE2)
						return -emsg(73);
		...
		parseescape:
			...
			if (FD_ISSET(commskt, &writequeue))
			{ /* check writing */
#ifdef DEBUG
				...
				case 1: /*  write header */
					...
					if (retval == -1)
						return -emsg(55);
					writeindex += retval;
					if (writeindex < (int)sizeof(shead))
						break;
					writeindex = 0;
