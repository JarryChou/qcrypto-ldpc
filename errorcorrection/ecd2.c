...
typedef struct header_7
{ /* header for type-7 stream packet */
    int tag;
    unsigned int epoc;
...
int insert_sendpacket(char *message, int length)
{
    ...
    if (lp)
        lp->next = newpacket; /* insetr in chain */
    last_packet_to_send = newpacket;
    if (!next_packet_to_send)
        next_packet_to_send = newpacket;
...
int check_epochoverlap(unsigned int epoch, int num)
{
    ...
    int en;
    while (bp)
    { /* as long as there are more blocks to test */
...
int create_thread(unsigned int epoch, int num, float inierr, float BellValue)
{
    ...
    for (enu = 0; enu < num; enu++)
    {
        ...
        if (h3.bitsperentry != 1)
            return 70; /* not a BB84 raw key */
        ...
        if (retval != i * sizeof(unsigned int))
            return 72; /* not enough read */
        ...
        if (h3.length & 0x1f)
        { /* we have some residual bits */
            newindex += (i - 1);
        }
        else
        { /* no fresh residue, old one stays as is */
            newindex += i;
        }
        if (resbitnumber > 31)
        {                                  /* write one in buffer */
    ...
    getbytes = newindex * 3 * sizeof(unsigned int) + bitcount * 2 * sizeof(unsigned short int);
    ...
    bp->next = blocklist;
    if (blocklist)
        blocklist->previous = bp; /* update existing first entry */
...
int remove_thread(unsigned int epoch)
{
    ...
    if (!bp)
        return 49; /* no block there */
    /* remove all internal structures */
    free2(bp->content->rawmem); /* bit buffers, changed to rawmem 11.6.06chk */
    if (bp->content->lp0)
        free(bp->content->lp0); /* parity storage */
    if (bp->content->diffidx)
        free2(bp->content->diffidx);
                                        ...
                                        float BellValue)
{
    ...
    { /* count through all needed bits */
        do
        { /* generate a bit position */
            bipo = PRNG_value2(rn_order, &kb->RNG_state);
            if (bipo > kb->initialbits)
                continue;        /* out of range */
            bpm = bt_mask(bipo); /* bit mask */
            if (kb->testmarker[bipo / 32] & bpm)
                continue; /* already used */
            /* got finally a bit */
            kb->testmarker[bipo / 32] |= bpm; /* mark as used */
            if (kb->mainbuf[bipo / 32] & bpm)
                localdata |= bt_mask(i);
            if ((i & 31) == 31)
            {
...
int errorest_1(unsigned int epoch)
{
    ...
    if (!(kb = get_thread(epoch)))
        return 73; /* cannot find key block */
...
int process_esti_message_0(char *receivebuf)
{
    ...
    if (overlapreply && in_head->seed)
        return 46; /* conflict */
    if ((!overlapreply) && !(in_head->seed))
        return 51;

    if (overlapreply)
    { /* we have an update message to request more bits */
        kb = get_thread(in_head->epoch);
        if (!kb)
            return 48; /* cannot find thread */
    ...
    if (in_head->errormode)
    { /* skip the error estimation */
        kb->errormode = 1;
    ...
    else
    {
        ...
        else
        {
            ...
            if (newbitsneeded > kb->initialbits)
            { /* will never work */
                replymode = 0;
            }
            else
            {
                if (newbitsneeded > kb->estimatedsamplesize)
                { /*  more bits */
                    replymode = 1;
                }
                else
...
void prepare_permut_core(struct keyblock *kb)
{
    ...
    for (i = 0; i < workbits; i++)
        kb->permuteindex[i] = 0xffff; /* mark unused */
    /* this routine causes trouble */
    for (i = 0; i < workbits; i++)
    { /* do permutation  */
        do
        { /* find a permutation index */
    ...
    { /*  do bit permutation  */
        k = kb->permuteindex[i];
        if (bt_mask(i) & kb->mainbuf[i / 32])
            kb->permutebuf[k / 32] |= bt_mask(k);
    }
...
void generate_BICONF_bitstring(struct keyblock *kb)
{
    ...
    { /* take care of the full bits */
        kb->testmarker[i] =
            PRNG_value2_32(&kb->RNG_state) & kb->permutebuf[i]; /* get permuted bit */
    }
    kb->testmarker[kb->workbits / 32] = /* prepare last few bits */
        PRNG_value2_32(&kb->RNG_state) & lastmask((kb->workbits - 1) & 31) & kb->permutebuf[kb->workbits / 32];
    return;
}
...
int prepare_dualpass(char *receivebuf)
{
    ...
    if (in_head->tested_bits != kb->leakagebits)
        return 52;
...
int prepare_first_binsearch_msg(struct keyblock *kb, int pass)
{
    ...
    {
        if (bt_mask(i) & pd[i / 32])
        {                                      /* this block is mismatched */
    ...
    kb->binsearch_depth = (pass == 0 ? RUNLEVEL_FIRSTPASS : RUNLEVEL_SECONDPASS) | 0; /* first round */
    ...
    for (i = 0; i < kb->diffnumber; i++)
        h5_idx[i] = kb->diffidx[i];

    /* prepare parity results */
    resbuf = 0;
    tmp_par = 0;
    for (i = 0; i < kb->diffnumber; i++)
    {                                                 /* go through all keyblocks */
...
void fix_parity_intervals(struct keyblock *kb, unsigned int *inh_idx)
{
    ...
    for (i = 0; i < kb->diffnumber; i++)
    { /* go through all different blocks */
        fbi = kb->diffidx[i];
        ...
        if (inh_idx[i / 32] & bt_mask(i))
        {                                               /* error is in upper (par match) */
...
void fix_permutedbits(struct keyblock *kb)
{
    ...
    if (kb->binsearch_depth & RUNLEVEL_LEVELMASK)
    { /* we are in pass 1 */
        src = kb->permutebuf;
        dst = kb->mainbuf;
        idx = kb->reverseindex;
    ...
    struct keyblock *kb, struct ERRC_ERRDET_5 *in_head)
{
    ...
    if (in_head->runlevel & RUNLEVEL_LEVELMASK)
    { /* this is pass 1 */
        d = kb->permutebuf;
        k = kb->k1;
    }
    ...
    kb->binsearch_depth = ((in_head->runlevel + 1) & RUNLEVEL_ROUNDMASK) + (in_head->runlevel & (RUNLEVEL_LEVELMASK | RUNLEVEL_BICONF));
    ...
    for (i = 0; i < kb->diffnumber; i++)
    {
        ...
        if (lbi < mbi)
        { /* end of interval, give zero parity */
            tmp_par = 0;
        }
                             ...
                             int lostbits)
{
    ...
    if (intrinsicerr < trueerror)
    {
	    ...
	    */
        ...
        { /* we do the device-indepenent estimation */
    ...
    if (kb->finalkeybits < 0)
        kb->finalkeybits = 0; /* no hope. */
    ...
    { /* no PA fo debugging */
        for (j = 0; j < numwords; j++)
            finalkey[j] = kb->mainbuf[j];
    }
    else
    { /* do privacy amplification */
        /* create compression matrix on the fly while preparing key */
        for (i = 0; i < kb->finalkeybits; i++)
        {          /* go through all targetbits */
            m = 0; /* initial word */
            for (j = 0; j < numwords; j++)
                m ^= (kb->mainbuf[j] &
                      PRNG_value2_32(&kb->RNG_state));
            if (parity(m))
                finalkey[i / 32] |= bt_mask(i);
        }
    }
    ...
    while (1)
    {
        ...
        if (rv == -1)
            return 65; /* write error happened */
        written += rv;
        if (written >= mlen)
            break;
    ...
    struct keyblock *kb, struct ERRC_ERRDET_5 *in_head)
{
    ...
    for (i = 0; i < kb->diffnumber; i++)
    {
        ...
        if (fbi == lbi)
        { /* end of interval, correct for error */
            if (biconfmark)
                correct_bit(d2, fbi);
            correct_bit(d, fbi);
            kb->correctederrors++;
    ...
    if ((kb->binsearch_depth & RUNLEVEL_ROUNDMASK) <
        get_order_2((kb->processingstate == PRS_DOING_BICONF) ? (kb->biconflength) : (thispass ? kb->k1 : kb->k0)))
    {
...
void dumpstate(struct keyblock *kb)
{
    ...
    if (kb->mainbuf)
        write(dha, kb->mainbuf, sizeof(unsigned int) * (2 * kb->initialbits + 3 * ((kb->initialbits + 31) / 32)));
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "c:s:r:d:f:l:q:Q:e:E:kJ:T:V:Ipb:B:i")) != EOF)
    {
        ...
        case 'J': /* error rate generated outside eavesdropper */
            if (1 != sscanf(optarg, "%f", &intrinsicerr))
                return -emsg(14);
            if ((intrinsicerr < 0) ||
                (intrinsicerr > MAX_INTRINSIC))
                return -emsg(15);
            break;
        case 'T': /* runtime error behaviour */
            if (1 != sscanf(optarg, "%d", &runtimeerrormode))
                return -emsg(16);
            if ((runtimeerrormode < 0) ||
                (runtimeerrormode > MAXRUNTIMEERROR))
                return -emsg(16);
            break;
    ...
    /* open pipelines */
    if (stat(fname[0], &cmdstat))
        return -emsg(18); /* command pipeline */
    if (!S_ISFIFO(cmdstat.st_mode))
        return -emsg(19);
    if (!(fhandle[0] = fopen(fname[0], "r+")))
        return -emsg(18);
    handle[0] = fileno(fhandle[0]);

    if (stat(fname[1], &cmdstat))
        return -emsg(20); /* send pipeline */
    if (!S_ISFIFO(cmdstat.st_mode))
        return -emsg(21);
    if ((handle[1] = open(fname[1], FIFOOUTMODE)) == -1)
        return -emsg(20);

    if (stat(fname[2], &cmdstat))
        return -emsg(22); /* receive pipeline */
    if (!S_ISFIFO(cmdstat.st_mode))
        return -emsg(23);
    if ((handle[2] = open(fname[2], FIFOINMODE)) == -1)
        return -emsg(22);
    ...
    if (stat(fname[6], &cmdstat))
        return -emsg(25); /* query pipeline */
    if (!S_ISFIFO(cmdstat.st_mode))
        return -emsg(26);
    if (!(fhandle[6] = fopen(fname[6], "r+")))
        return -emsg(25);
        ...
        { /* there was a pending request */
            ...
            if (FD_ISSET(handle[2], &readqueue))
            {
                if (receive_index < sizeof(struct ERRC_PROTO))
                {
                    ...
                    if (retval == -1)
                        return -emsg(36); /* can that be better? */
